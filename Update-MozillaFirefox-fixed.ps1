...
Updated by Paul Davis to fix bug in latest 64-bit FF install
...
Function Check-InstalledSoftware ($display_name) {
    Return Get-ItemProperty $registry_paths -ErrorAction SilentlyContinue | Where-Object { $_.PSChildName -like $display_name }
} # function
        ...
        If ($powershell_v2_or_earlier -eq $true) {
            ...
            catch
            {
                ...
                Write-Verbose $message
							...
							# When testing this out repeatedly, variable need reset
                            If (Test-Path variable:currently_installed_64)
                            {
							    Clear-Variable -Name ("currently_installed_64")
                            }
...
If ($firefox_is_installed -eq $true) {
    ...
    Return "No Firefox seems to be installed on the system."
} # Else (No Firefox )
...
# I don't think we need to stop these
        ...
        If ($powershell_v2_or_earlier -eq $true) {
            ...
            catch
            {
                ...
                Write-Verbose $message
							...
							# When testing this out repeatedly, variable need reset
                            If (Test-Path variable:obj_success_firefox)
                            {
							    Clear-Variable -Name ("obj_success_firefox")
                            }
